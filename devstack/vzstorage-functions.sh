...
function install_vzstorage {
    ...
    echo PASSWORD | sudo pstorage -c $CLUSTER_NAME make-mds -I -a 127.0.0.1 \
        -r $VZSTORAGE_DATA_DIR/$CLUSTER_NAME-mds -P
    sudo service pstorage-mdsd start
    sudo chkconfig pstorage-mdsd on

    sudo pstorage -c $CLUSTER_NAME make-cs -r $VZSTORAGE_DATA_DIR/$CLUSTER_NAME-cs
...
function configure_cinder_backend_vzstorage {
    ...
    CINDER_VZSTORAGE_CLUSTERS="$VZSTORAGE_CLUSTER_NAME [\"-u\", \"stack\", \"-g\", \"qemu\", \"-m\", \"0770\"]"
